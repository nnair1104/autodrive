...
FreicarSignDetect::FreicarSignDetect(const std::unique_ptr<ros::NodeHandle> &nodehandle, const std::string agent_name): image_transport_(*nodehandle)
{
    ...
    image_sub_  = std::make_unique<image_transport::Subscriber>(image_transport_.subscribe(agent_name_ + "/sim/camera/rgb/front/image", 1, &FreicarSignDetect::ImageCallback, this));
